...
import { gmeDataTs } from './gme-data-small';
...
export class ImportGmeDataService {
  ...
  private _priceEntriesFilled: GmePriceEntrySimple[] = [];
  private _tradingDayPriceEntries: GmePriceEntrySimple[] = [];
  public get allPriceEntries(): GmePriceEntrySimple[] { return this._priceEntriesFilled; }
  public get tradingDayPriceEntries(): GmePriceEntrySimple[] { return this._tradingDayPriceEntries; }
  ...
  public setGmePriceEntries(entries: GmePriceEntrySimple[]) { this._priceEntriesFilled = entries; }
  ...
  //   const gmeData: GmePriceEntrySimple[] = gmeDataTs.map(tsData => {
  //     const newEntry: GmePriceEntrySimple = {
  //       dateYYYYMMDD: this._convertToDate(tsData.date),
  //       volume: tsData.volume,
  //       close: Number(tsData.close.substring(1)),
  //     }
  //     return newEntry;
  //   });


  //   return this._sortData(gmeData);
  // }
  ...
  private async _loadGMECSVdataCurrentEra$() {
    const subject$ = new Subject<GmePriceEntrySimple[]>();
    const gmeDatafileName = 'assets/data/gme-data.csv';

    const gmeDataTsImport = gmeDataTs;
  ...
  private _parseCSV(data: any): GmePriceEntrySimple[] {
    // console.log(data);
    const rows = data.split('\n');
    const headers = rows[0].split(';');
    const rowCount = rows.length - 1;
    const priceEntries: GmePriceEntrySimple[] = [];
    ...
    for (let rowIndex = 1; rowIndex < rowCount; rowIndex++) {
      ...
      const priceEntry: GmePriceEntrySimple = {
  ...
  private _mergeEntries(csvEntries: GmePriceEntrySimple[], sheetEntries: GmePriceEntrySimple[]): GmePriceEntrySimple[] {
    const allEntries: GmePriceEntrySimple[] = Object.assign([], csvEntries);
  ...
  private _sortData(priceEntries: GmePriceEntrySimple[]): GmePriceEntrySimple[] {
   ...
   */
  private _fillGaps(priceEntries: GmePriceEntrySimple[], isForTradingDays: boolean): GmePriceEntrySimple[] {
    ...
    const newEntries: GmePriceEntrySimple[] = [];
